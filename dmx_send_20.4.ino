/*
  ...
  Teensy 3.2 - 72 Mhz - Fast - MIDI - German Keyboard
*/
#include "effects.h"
...
#define pinB           3
#define Switch         4
#define writeA(r, g, b, w, x)  dmx.set(4, x); dmx.set(5, r); dmx.set(6, g); dmx.set(7, b); dmx.set(8, w);     //dim, r, g, b, w
...
#define writeHead(s, x)        dmx.set(67, x); dmx.set(73, s);  //sound, dim
#define writeFog(t, f)         dmx.set(33, t); dmx.set(34, f);  //temp, fog
#define writeStrobe(s, d)      dmx.set(36, s); dmx.set(37, d)   //speed, dim
#define writePanel(r, g, b, x) dmx.set(38, x); dmx.set(39, r); dmx.set(40, g); dmx.set(41, b);  //dim, r, g, b
...
rampByte RP;  rampByte GP;  rampByte BP;
...
////////////RAMP/////////////////
byte rLast[5] = {0, 0, 0, 0, 0};  //Ramp
byte gLast[5] = {0, 0, 0, 0, 0};
byte bLast[5] = {0, 0, 0, 0, 0};
byte wLast[4] = {0, 0, 0, 0};
byte rOut[5] = {0, 0, 0, 0, 0};
byte gOut[5] = {0, 0, 0, 0, 0};
byte bOut[5] = {0, 0, 0, 0, 0};
byte wOut[4] = {0, 0, 0, 0};
...
byte exStSpeed, exStDim; //extern Strobe
...
OBJ * currentPage = &musicSetting; //page that is diplayed
...
void assignment(){
  ...
  musicSetting = {{"MIDI", "Zufall", "AUX"}, {false, false, false}, &musicSetting};         //Aux is not yet implemented
  autoSetting = {{"Langsam", "Normal", "Schnell"}, {false, false, false}, &autoSetting};
...
OBJ * subMenuHandling(OBJ * newPage, byte newElement){
    ...
    if(music_0[0]) MIDIrandom = false; //Midi on
    if(music_0[1]) MIDIrandom = true; //random on 
    if(music_0[2]) MIDIrandom = true; //random on, because aux is not yet implemented
    if(auto_0[0]){ speedAdjust = true; speedAdjustFactor = 2;} //slow
    if(auto_0[1]) speedAdjust = false; //normal
    if(auto_0[2]){ speedAdjust = true; speedAdjustFactor = 0.5;} //rapid
...
void doMidiMode(byte dimVal){
  if(clkCount >= beatTreshold[beat]){
    ...
    writePanel(panel[curFx][0][cntFx], panel[curFx][1][cntFx], panel[curFx][2][cntFx], dimVal)
  }

}

void doRandomMode(byte dimVal, int timeVal){
  if(timeVal > musicTreshold){
    musicTreshold = constrain(analogRead(A6), 250, 1023);
    cntFx ++;
...
void writeMainOutput(byte dimVal){
  ...
  rOut[4] = check(RP.update(), rLast[4]);        
  if(rOut[4] > 0) rLast[4] = rOut[4] -1;
  gOut[4] = check(GP.update(), gLast[4]);        
  if(gOut[4] > 0) gLast[4] = gOut[4] -1;
  bOut[4] = check(BP.update(), bLast[4]);        
  if(bOut[4] > 0) bLast[4] = bOut[4] -1;  
  writePanel(rLast[4], gLast[4], bLast[4], dimVal)
} //writeOutput

void writeSubOutput(){
  soundSens = map(analogRead(A0), 0, 1023, 210, 255); //moving head sound sensetivity
  if(soundSens > 250) soundSens = 255;
  if(soundSens < 215) soundSens = 210;
  headDim = map(analogRead(A1), 0, 1023, 0, 255); //moving head dim
  if(headDim > 245) headDim = 255;
  if(headDim < 15) headDim = 0;
  writeHead(soundSens ,headDim);

  exStSpeed = map(analogRead(A2), 0, 1023, 0, 255); //extern strobe
  if(exStSpeed > 250) exStSpeed = 255;
  if(exStSpeed < 10) exStSpeed = 0;
  exStDim = map(analogRead(A3), 0, 1023, 0, 255);
  if(exStDim > 250) exStDim = 255;
  if(exStDim < 10) exStDim = 0;   //extern Strobe
  
  temp = map(analogRead(A4), 0, 1023, 0, 255);  //turn heater on or off
  fog = map(analogRead(A5), 0, 1023, 0, 255);   //release fog
  writeFog(temp, fog);
}
...
void doStrobeMode(){
  ...
  writeStrobe(exStSpeed, exStDim); //extern Strobe
}

void doBlackMode(){
  notNormalUse = true;
  pauseRamp();
  dmx.set(4, 0); dmx.set(12, 0); dmx.set(20, 0); dmx.set(28, 0); dmx.set(38, 0); dmx.set(67, 0); //brigthness = 0
  //lamp1       lamp2            lamp3           lamp4           panel           moving head
  
  writeStrobe(0,0); //extern strobe off (just in case)
}
...
void normalUse(){
  ...
  writeStrobe(0,0);   //extern strobe off
}
...
void setRamp(){
    ...
    RP.go(panel[curFx][0][cntFx], dur);
    GP.go(panel[curFx][1][cntFx], dur);
    BP.go(panel[curFx][2][cntFx], dur);
} //setRamp()

void pauseRamp(){
if(!BB1.isPaused()){ 
    R1.pause(); R2.pause(); R3.pause(); R4.pause(); RP.pause();
    G1.pause(); G2.pause(); G3.pause(); G4.pause(); GP.pause();
    BB1.pause(); B2.pause(); B3.pause(); B4.pause(); BP.pause();
...
void resumeRamp(){
  if(BB1.isPaused()){
    R1.resume(); R2.resume(); R3.resume(); R4.resume(); RP.resume();
    G1.resume(); G2.resume(); G3.resume(); G4.resume(); GP.resume();
    BB1.resume(); B2.resume(); B3.resume(); B4.resume(); BP.resume();
...
void setup() {
  ...
  tft.fillScreen(ST77XX_BLACK);   //begin: startup message
  tft.setTextColor(ST77XX_WHITE);  
  tft.setCursor(20, (40 * 0) + 10);
  tft.print("Bitte den");
  tft.setCursor(20, (40 * 1) + 10);
  tft.print("Musikmodus");
  tft.setCursor(20, (40 * 2) + 10);
  tft.print("auswaehlen");
  delay(5000);                     //end: startup message
